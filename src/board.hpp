...
class Board {
    ...
    std::stack<int> moveStack;  //stores the squares that are modified. this is used by undo for udndoing the move

    //representation of state: [KQkq- 1 bit for each][enpassant possible-1 bit][3 bits for the column if enpassant possible]
    ...
    void fromFEN(std::stringstream& ss);                  //resets the board to the given fen
    ...
    char& operator[](int i); 
    void print();   //prints the board. used for debugging
    void undo();    //undoes the last move using the moveStack
};
